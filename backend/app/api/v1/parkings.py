...
async def create_parking(parking_in: ParkingCreate, db: AsyncSession = Depends(get_db), current_user = Depends(write_required)):
    ...
    db_parking = Parking(
        ...
        billing_unit=parking_in.billing_unit or "hour",
        rate_minute_auto=parking_in.rate_minute_auto if parking_in.rate_minute_auto is not None else 0.08,
        rate_minute_suv=parking_in.rate_minute_suv if parking_in.rate_minute_suv is not None else 0.12,
        rate_minute_mototaxi=parking_in.rate_minute_mototaxi if parking_in.rate_minute_mototaxi is not None else 0.06,
        rate_minute_moto=parking_in.rate_minute_moto if parking_in.rate_minute_moto is not None else 0.04,
        rate_monthly_auto=parking_in.rate_monthly_auto if parking_in.rate_monthly_auto is not None else 180.0,
        rate_monthly_suv=parking_in.rate_monthly_suv if parking_in.rate_monthly_suv is not None else 240.0,
        rate_monthly_mototaxi=parking_in.rate_monthly_mototaxi if parking_in.rate_monthly_mototaxi is not None else 120.0,
        rate_monthly_moto=parking_in.rate_monthly_moto if parking_in.rate_monthly_moto is not None else 90.0,
        rate_monthly=parking_in.rate_monthly if parking_in.rate_monthly is not None else (parking_in.rate_monthly_auto or 180.0),
        min_stay_minutes=int(parking_in.min_stay_minutes or 15),
        max_stay_minutes=int(parking_in.max_stay_minutes or 1440),
