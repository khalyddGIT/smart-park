...
export const PlatformGlobalDashboard = ({ onNavigateTab }) => {
  ...
  const liveEvents = liveEventsReal.length ? liveEventsReal : [
    { id: 1, type: 'LPR_ENTRY', title: 'Sin eventos recientes', desc: 'La bitácora registrará la actividad en tiempo real', time: 'En espera', badge: 'Sistema', color: 'emerald' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 dark:bg-[#0E1526] text-white p-6 sm:p-7 rounded-3xl shadow-sm border border-slate-800/80 flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2.5 relative z-10">
          <Button
            ...
            className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-2xl shadow-sm gap-1.5 py-5 px-4 cursor-pointer transition-colors"
          >
      ...
      {/* Bento Grid de Métricas Principales (Diseño Asimétrico y Jerarquizado) */}
      {isRefreshing ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
          ...
          {/* Card Hero Financiero: Recaudación Bruta + Comisión de Plataforma (Spans 5 cols) */}
          <div className="lg:col-span-5 p-5 rounded-3xl border border-slate-200/90 dark:border-slate-800/80 bg-white dark:bg-[#111827] shadow-xs flex flex-col justify-between relative overflow-hidden group">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/70">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Rendimiento Financiero</span>
              </div>
            ...
            <div className="my-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:border-l sm:border-slate-100 dark:sm:border-slate-800/80 sm:pl-4">
                <span className="text-[11px] text-emerald-700 dark:text-emerald-400 font-semibold block">Comisión Neta (12%)</span>
          ...
          {/* Card Ocupación y Aforo (Spans 3 cols) */}
          <div className="lg:col-span-3 p-5 rounded-3xl border border-slate-200/90 dark:border-slate-800/80 bg-white dark:bg-[#111827] shadow-xs flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Ocupación Red</span>
            ...
            <div className="my-2">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">de capacidad</span>
              </div>

              <div className="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden mt-3">
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${occupancyPercentage > 85 ? 'bg-rose-500' : 'bg-emerald-500'}`} 
                  style={{ width: `${occupancyPercentage}%` }} 
                />
              </div>

              <div className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400 mt-2 font-mono">
                <span>{occupiedSlotsCount} ocupadas</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold">{freeSlotsCount} libres</span>
              </div>
            </div>

            <div className="pt-2 border-t border-slate-100 dark:border-slate-800/70 text-[11px] text-slate-400 dark:text-slate-500">
              Total {totalSlotsCount} plazas registradas
            </div>
          </div>

          {/* Card Sedes Afiliadas (Spans 2 cols) */}
          <div className="lg:col-span-2 p-5 rounded-3xl border border-slate-200/90 dark:border-slate-800/80 bg-white dark:bg-[#111827] shadow-xs flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Cocheras</span>
              <div className="w-7 h-7 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center">
          ...
          {/* Card Reservas & Reputación (Spans 2 cols) */}
          <div className="lg:col-span-2 p-5 rounded-3xl border border-slate-200/90 dark:border-slate-800/80 bg-white dark:bg-[#111827] shadow-xs flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Actividad</span>
              <div className="w-7 h-7 rounded-xl bg-amber-500/15 dark:bg-amber-500/20 text-amber-500 flex items-center justify-center">
            ...
            <div className="my-2">
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium block mt-0.5">Reservas registradas</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 rounded-3xl border-slate-200/90 dark:border-slate-800/80 shadow-xs bg-white dark:bg-[#111827] space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800/80 pb-3">
            <div>
              <h2 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                <BarChart3 className="w-5 h-5 shrink-0 text-emerald-500" />
                <span>Recaudación por Sede</span>
              </h2>
        ...
        <Card className="p-6 rounded-3xl border-slate-200/90 dark:border-slate-800/80 shadow-xs bg-white dark:bg-[#111827] space-y-4 flex flex-col justify-between">
          <div className="border-b border-slate-100 dark:border-slate-800/80 pb-3">
            <h2 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
              <CreditCard className="w-5 h-5 shrink-0 text-emerald-500" />
              <span>Resumen Consolidado</span>
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Balance global del ecosistema.</p>
          </div>

          <div className="space-y-3 text-xs">
            <div className="flex justify-between p-3.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800/80">
            ...
            <div className="flex justify-between p-3.5 bg-emerald-500/10 rounded-2xl border border-emerald-500/20">
              <span className="text-emerald-800 dark:text-emerald-300 font-bold">Comisión Plataforma (12%)</span>
              <strong className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">S/ {Number(netCommissionToday).toFixed(2)}</strong>
            </div>
            <div className="flex justify-between p-3.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800/80">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-800/80">
            Liquidación automatizada del 12% y conciliación en tiempo real.
          </p>
        </Card>
      </div>

      {/* Monitor de Cocheras y Live Feed */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 rounded-3xl border-slate-200/90 dark:border-slate-800/80 shadow-xs bg-white dark:bg-[#111827] space-y-4">
          ...
          <div className="space-y-3">
            {branchPerformance.map((b) => (
              <div key={b.id} className="p-4 rounded-2xl border border-slate-100 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-white dark:hover:bg-slate-800/70 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 self-end sm:self-auto">
                  ...
                  <div className="text-right font-mono border-l border-slate-200 dark:border-slate-700/80 pl-3">
        ...
        <Card className="p-6 rounded-3xl border-slate-200/90 dark:border-slate-800/80 shadow-xs bg-white dark:bg-[#111827] space-y-4">
          <div className="border-b border-slate-100 dark:border-slate-800/80 pb-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Registro de actividad y accesos vehiculares en la red.</p>
            </div>
          ...
          <div className="space-y-3">
            {liveEvents.map((ev) => (
              <div key={ev.id} className="p-3.5 rounded-2xl bg-slate-50/70 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/70 space-y-1 hover:bg-white dark:hover:bg-slate-800/60 transition">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border ${
                    ev.color === 'emerald' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/25' :
                    ev.color === 'amber' ? 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/25' :
